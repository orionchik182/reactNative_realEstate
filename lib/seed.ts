...
import { databases, config, tables } from "./appwrite2";
import {
  agentImages,
  galleryImages,
  propertiesImages,
...
async function seed() {
  try {
    ...
    for (const key in TABLES) {
      ...
      // Получаем список строк (listRows)
      const rows = await tables.listRows({
        databaseId: config.databaseId!,
        tableId: tableId!,
      });

      // Удаляем каждую строку (deleteRow)
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      const property = await tables.createRow({
        ...
        data: {
          ...
          image: image,
          agent: assignedAgent.$id, // Связь с агентом
          reviews: assignedReviews.map((review) => review.$id), // Массив ID отзывов
          gallery: assignedGalleries.map((gallery) => gallery.$id), // Массив ID галереи
        },
      });
